...
class DDIM:
    ...
    def linear(self, z1, z2, alpha):
            return (z2 * alpha) + (z1 * (1 - alpha))

    def load_all_noise(self, noise_folder, function=0): #0:slerp, 1:linear
        ...
        alpha = [i / 10.0 for i in range(11)]
        final_noise = []
        if function == 0:
            
            for i in range(len(alpha)):
                final_noise.append(self.slerp(noise_batch[0], noise_batch[1], alpha[i]))
            
            
        elif function == 1:
            for i in range(len(alpha)):
                final_noise.append(self.linear(noise_batch[0], noise_batch[1], alpha[i]))

        return torch.cat(final_noise, dim=0), noise_files
...
if __name__ == "__main__":
    ...
    noise_batch, noise_files = ddim.load_all_noise(noise_folder, function=function)
    ...
    for idx, img in enumerate(generated_imgs):
        ...
        spreadsheet.append(img)
        generated_imgs[idx] = img

    spreadsheet = torch.cat(spreadsheet, dim=2)
    save_image(spreadsheet, f"{save_path}/concat.png")
